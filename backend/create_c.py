...
def assign_teachers_shift_wise():
    ...
    for teacher in teachers_with_classes:
        try:
            ...
            # Convert campus string to Campus object if needed
            if isinstance(teacher_campus, str):
                try:
                    # Extract campus number from string like "Campus 1 (KAR-OFUD)"
                    import re
                    campus_match = re.search(r'Campus (\d+)', teacher_campus)
                    if campus_match:
                        campus_number = int(campus_match.group(1))
                        teacher_campus = Campus.objects.get(id=campus_number)
                    else:
                        teacher_campus = Campus.objects.get(campus_name=teacher_campus)
                except Campus.DoesNotExist:
                    print(f"  Campus not found: {teacher_campus}")
                    error_count += 1
                    continue
...
def main():
    ...
    for classroom in sample_classes:
        ...
        # Get student count from Student model
        from students.models import Student
